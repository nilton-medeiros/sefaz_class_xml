...
method new() class TICMS20
	::documentation := "ICMS20 | Prestação sujeito à tributação com redução de BC do ICMS"
	::submit := False
	::value := "TICMS20"
	::CST := Telement():new({'name' => "CST", 'documentation' => "Classificação Tributária do Serviço"+CRLF+"20 - tributação com BC reduzida do ICMS",;
												'value' => "20", 'minLength' => 2, 'type' => "N"})
	::pRedBC := Telement():new({'name' => "pRedBC", 'documentation' => "Percentual de redução da BC", 'minLength' => 4, 'maxLength' => 6, 'type' => "N"})
